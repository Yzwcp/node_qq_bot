...
class MessageDeal{
    ...
    consumptionAndSavings(){
        ...
        /*存账号*/
        if(Number(textList[0])===20){
            this.reqServer('setpwd',commandStr,(data)=>{

                let { acc,pwd,desc,url ='' } = data
                return [
                    `--设置账号--`,
                    `描述：${desc}`,
                    `账号：${acc}`,
