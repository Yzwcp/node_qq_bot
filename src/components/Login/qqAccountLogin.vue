<template>
    <el-card>
        <el-form label-width="100px" :model="formData" style="max-width: 800px" :label-suffix="':'">
            <el-form-item label="登录标识" prop="account">
                <el-select v-model="formData.platform" class="m-2" placeholder="选择">
                    <el-option v-for="item in platFormSelect" :key="item.v" :label="item.l" :value="item.v" />
                </el-select>
            </el-form-item>
            ...
            <el-form-item label="操作1">
                ...
                <div v-if="webSocket.code === BotEvent.on.loginSlider" style="margin-left: 10px; color: skyblue;"><a
                        :href="webSocket.data?.ticket" target="_blank">已获取:点击去提取</a></div>
            </el-form-item>
            ...
            <el-form-item label="验证码" prop="verificationCode" v-if="webSocket.code==='needCode'">
                <el-input v-model="formData.verificationCode" style='width: 200px;'>
                    <template #append>
                        <el-button @click="getVerificationCode" :disabled="false" type="primary">
                            {{countdown==60?'获取验证码':countdown}}
                        </el-button>
                    </template>
                </el-input>
                <el-icon size="16px" style="margin:0px 4px 0 10px;color: red;display: inline-block;">
                    <Warning />
                </el-icon>
                <span style="color: red;">首次登录新设备需要获取密保手机验证码(验证完以后可以直接登录)</span>

            </el-form-item>
            <el-form-item label="操作3" v-if="webSocket.code==='needCode'">
                <el-button @click="replyVerificationCode" type="primary">登录</el-button>
            </el-form-item>
        </el-form>
    </el-card>
</template>
...
const replyVerificationCode = () => {
    ...
    webSocket.send({
        data: {
            verificationCode: formData.verificationCode, account: formData.account,
            password: formData.password,
            platform: formData.platform
